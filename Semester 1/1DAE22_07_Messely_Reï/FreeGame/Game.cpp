...
void Draw()
{
	...
	if (!g_IsMainMenuActive)
	{
		for (int i{ 0 }; i < g_AmountOfBoxes; i++)
		{
			...
			if (g_Boxes[i].isVisible) 
			{
				SetColor(g_Boxes[i].color);
...
{
	if(CheckInsideRect(box.rect.left - g_MainBallRadius, box.rect.bottom - g_MainBallRadius, box.rect.width + g_MainBallRadius, box.rect.height + g_MainBallRadius, ballPos))
	{
		if (box.isVisible) 
		{
			float offSet{ 2 };
...
void InitialiseBoxes()
{
	...
	for (unsigned int i{ 0 }; i < g_AmountOfBoxes; i++)
	{
		...
		g_Boxes[i].isVisible = true;
#pragma region Color
		if ((i % 36) < 12) g_Boxes[i].color = g_SoftGreen;
		else if ((i % 36) < 24) g_Boxes[i].color = g_SoftPink;
		else if ((i % 36) < 36) g_Boxes[i].color = g_SoftYellow;
#pragma endregion
	}
}
...
void ShowInfo()
{
	std::cout
		<< "\n== BREAK IN INFO ==\n"
		<< "You have 5 balls and need to break all the blocks as fast as possible\n"
...
void DrawLivesOnScreen()
{
	std::string lives{ std::to_string(g_Lives) + " lives"};
	for (int i{0}; i < 8; i++) 
	{
		const Point2f locL{ 5.0f+i*10,5.0f };
		FontLoader(lives[i], locL, 2, 2, 2);
	}
